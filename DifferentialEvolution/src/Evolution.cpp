...
void Evolution::set_seed(unsigned int seed)
{
  srand(seed);
  return;
}

void Evolution::set_wt_factor(double wt_factor)
{
  this->wt_factor = wt_factor;
  return;
}

void Evolution::toggle_display()
{
  display = not display;
}
...
{
  this->extra      = extra;                  // passed on to objective
  this->nv         = nv;                     // number of variables
  this->nm         = nm;                     // number of members
  max_gen          = 50;                     // maximum generations allowed
  wt_factor        = 0.85;                   // weighting factor
...
void Evolution::evolve(double* lower, double* upper, double* phi )
{
  ...
  int     best  = find_best();
  bool    done  = false;
  while ( (gen < max_gen) and !done )
  {
    if(display)
      fprintf(stdout, "%10d%10d%15.6g\n", gen, best, value[best]);
...
void Evolution::create_next()
{
  ...
  for (int j = 0; j < nm; j++)
    ...
    for(int i=0; i<nv; i++)
      if(uniform() <= mut)
        trial[i] = xj[i] + wt_factor*(b[i] - xj[i]) - wt_factor*(x1[i] - x2[i]);
      else
        trial[i] = xj[i] + wt_factor*(b[i] - xj[i]) + wt_factor*(x1[i] - x2[i]);
